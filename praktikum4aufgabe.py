...
# Thyristorsteller-Kennwerte
# TODO: Werte mit LibreOffice ausrechnen und einfügen
#THYRISTOR_Y_OFFSET =     # Y-Achsenabschnitt
#THYRISTOR_Y_MAX    =     # Letzter "Sicherer" Wert nach oben (aus dem Diagramm)
#THYRISTOR_X_STEIGUNG =   # Steigung

AO_MIN, AO_MAX = 0.0, 10.0  # DAQ-Ausgangsgrenzen [in Volt] für den Thyristorsteller
...
def spannung_zu_druck_mbar(u):
    """
    Rechnet die Sensorspannung des Drucksensors (u, in Volt) in einen
    Druck in mbar um.
    """
...
def spannung_zu_temperatur(u):
    """
    Rechnet eine Sensorspannung (u, in Volt) des AD8495-Verstärkers in
    eine Temperatur in Grad Celsius um.
    """
    # TODO: eigenen Code hier einfügen
    pass
...
def last_zu_spannung(last_pct):
    """
    Rechnet eine Stellgröße in Prozent (last_pct, 0-100 %) anhand der
    Thyristorsteller-Kennlinie in eine Steuerspannung in Volt um.

    Formel aus Datenblatt mit LibreOffice/Excel selbst herleiten

    Zusätzlich zwei Sicherheitsgrenzen beachten:
      - Ist die berechnete Spannung kleiner als der unterste "sichere Wert"
        (vom Datenblatt abgelesen), gib stattdessen AO_MIN zurück
        (Thyristorsteller schaltet erst ab diesem Wert überhaupt durch)
      - Die berechnete Spannung darf nie größer als der oberste "sichere Wert"
        werden

    TODO: Implementiere die Formel inkl. der beiden Sicherheitsgrenzen
    und gib den passenden Spannungswert zurück.
    """
    pass
